...
SEXP gridMGC_c(SEXP map, SEXP gc,SEXP ngrid)
{
  double *M=REAL(map),*GC=REAL(gc);
  ...
  SEXP res;
  PROTECT(res=allocVector(REALSXP,5*ng+50));
  ...
  for(i=0;i<k;i++)
    {
      REAL(res)[1+5*i]=Ngrid[i];
      REAL(res)[2+5*i]=Mgrid_l[i];
      REAL(res)[3+5*i]=Mgrid_u[i];
      REAL(res)[4+5*i]=GCgrid_l[i];
      REAL(res)[5+5*i]=GCgrid_u[i];
      //Rprintf("%d\t%lf\t%lf\t%lf\t%lf\t%lf\n",i,Ngrid[i],Mgrid_l[i],Mgrid_u[i],GCgrid_l[i],GCgrid_u[i]);
    }
...
SEXP gridM_c(SEXP map,SEXP ngrid)
{
  double *M=REAL(map);
  ...
  for(i=0;i<k;i++)
    {
      ...
      REAL(res)[2+3*i]=Mgrid_l[i];
      REAL(res)[3+3*i]=Mgrid_u[i];
      //Rprintf("%d\t%lf\t%lf\t%lf\n",i,Ngrid[i],Mgrid_l[i],Mgrid_u[i]);
    }
